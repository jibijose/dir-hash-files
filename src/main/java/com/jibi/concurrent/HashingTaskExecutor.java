...
public class HashingTaskExecutor {
    ...
    public List<File> executeFileHashing(String directory, Map<String, FileInfo> mapExistingFileInfos, Collection<FileInfo> listFileInfos, HashOperation hashOperation) {
        ...
        for (File file : files) {
            if (file.isDirectory() && !Files.isSymbolicLink(file.toPath())) {
                if (!FileUtil.isTempDirectory(file.getPath(), true)) {
                    fileList.addAll(executeFileHashing(file.getPath(), mapExistingFileInfos, listFileInfos, hashOperation));
                }
        ...
        if (listInsideFiles.size() > 0) {
            ...
            HashingTask hashingTask = new HashingTask(inDirFiles, dirValuePrefix, mapExistingFileInfos, listFileInfos, hashOperation);
